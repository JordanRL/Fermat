...
So then, why ever use immutable objects? Well, consider the next example.
...
#### ImmutableDecimal Implementation of `setValue()`
...
#### MutableDecimal Implementation of `setValue()`
...
The downside to this is that the newly created object is not referenced anywhere except in the return value. Without a reference, the object becomes inaccessible if you do not assign the returned result to a value in the calling scope.
...
- The number represents a concrete state, and the result of the equation will be a derived or dynamically generated value that doesn't actually exist within the data, such as using a population value to estimate the average wealth of a given individual
- The number represents a value that may be used multiple times for separate calculations, such as the mean in a normal distribution
